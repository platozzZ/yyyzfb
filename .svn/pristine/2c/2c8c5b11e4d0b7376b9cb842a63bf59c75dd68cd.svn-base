...
<view class="min-collapse">
  ...
  <view class="card" >
      <view class="card-view" @click="test" >
        <minDescInput style="width: 100%;" :desc="desc"  v-model="value" :sign="sign" :border="border"  :placeholder="placeholder" :disabled="disabled"></minDescInput>
        <!-- <image class="right-arrow p-left-10"  :src="isShow ? '../../static/images/downarrow24.png' : '../../static/images/arrow.png'" /> -->
        <image class="right-arrow "  :class="isShow ? 'animation' : 'right-arrow-a animation' "  src="/static/min-collapse/arrow.png" />
      </view>
      <view v-if="isArray">
      ...
      <view v-if="!isArray" >
        <view class="content-txt" :class="isShow ? 'animation ': ' animation' " :style="{'maxHeight':isShow? '1000rpx' :'0'}">
          <slot class="na"></slot>
        </view>
      </view>
