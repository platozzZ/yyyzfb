...
export default {
	...
	methods: {
		...
		insertImage() {
			...
			uni.chooseImage({
				...
				success: async(res) => {
					...
					that.getqntoken(res.tempFilePaths[0],md5(Math.random().toString(36).substr(2)))
		...
		async getqntoken(poster, key){//得到七牛云token
			var that = this;
			uni.showLoading({
				title: '正在上传中...'
			})
		...
		upload(filePath,keys,ind){
			...
			qiniuUploader.upload(filePath, (res) => {
				that.editorCtx.insertImage({
					src: `http://rdata-image.fblife.com/${res.key}`,
					alt: '图片',
					success: function(e) {}
				});
				uni.hideLoading()
