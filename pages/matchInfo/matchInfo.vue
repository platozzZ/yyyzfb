<template>
	<view class="matchInfo">
		<cu-custom bgColor="bg-white" :isLeft="true" :isRight="true">
			<block slot="left">
				<text class="cuIcon-homefill bar-action" @tap="closeWebview" v-if="fromFb == '1'"></text>
				<text class="cuIcon-back bar-action" @tap="backPage" v-else></text>
			</block>
			<block slot="content">
		...
		<image class="image-bg" :src="matchInfoDatas.poster" :style="{ top : `${containerTop}px` }" mode="aspectFill"></image>
	...
	export default {
		data() {
			return {
				...
				tabActiveIdx:0,
				containerTop: this.StatusBar + 45
			};
		},
		methods:{
			closeWebview(){
		...
		onLoad(e) {
			console.log('matchinfo:',e);
			this.mid = e.mid;
			var that = this;
			that.fromFb = e.fromFb
			uni.getSystemInfo({
